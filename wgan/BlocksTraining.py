...
if __name__ == '__main__':
    ...
    mapper.map_to_super_small_embedding('../data/small_dims_parts', '../data/small_dims_part_mapped')
    ...
    parser.add_argument("--embedding_height", type=int, default=9, help="height of an embedding")
    ...
    transformer.transform_dataset('../data/small_dims_part_mapped',
                                  '../data/small_dims_part_mapped_transformed')
    print("DATASET TRANSFORMATION FINISHED")

    dataloader = torch.utils.data.DataLoader(
        NNEmbeddingDataset('../data/small_dims_part_mapped_transformed'),
    ...
    os.makedirs('small_dims_examples', exist_ok=True)
    epochs = []
    losses_G = []
    losses_D = []
    batches_cnt = 0
        ...
        for i, objs in enumerate(dataloader):
            ...
            if batches_cnt % options.sample_interval == 0:
                ...
                for jj in range(len(fake)):
                    ...
                    fake[jj] = mapper.de_map_from_super_small_embedding(fake[jj], [1, 28])
                with open('small_dims_examples/generated_example' + str(batches_cnt), 'w+') as f:
    ...
    plt.savefig('Small Dim GAN parts losses')
    ...
    #             print(fake[i], file=f)
    torch.save(generator.state_dict(), './small_dims_generator_weights')
    torch.save(discriminator.state_dict(), './small_dims_discriminator_weights')
